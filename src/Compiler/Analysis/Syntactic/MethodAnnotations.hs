...
extract :: [Declaration] -> [(Name, Sigma'Type, Name, Name)]
...
-}
collect :: Declaration -> Maybe [(Name, Sigma'Type, Name, Name)]
  ...
  = return $ mapMaybe (fmap qualify'method . Annotations.collect) declarations
    where
      ...
      qualify'method (name, T'Forall tvs (context :=> type'))
        = (name, T'Forall tvs ((Is'In cl'name (T'Var' t) : context) :=> type'), var'name, cl'name)
collect _
  = Nothing
